...
{
  MaxDistComputerOpt2 comp{domain, f};
  return comp.computeAttribute(tree);
}

MaxDistComputerOpt2::MaxDistComputerOpt2(const Box &domain,
  const std::vector<uint8> &f)
  : domain_{domain}, f_{f},
    OFFSET_NEIGHBOR{ {-1, 0}, {0, -1}, {1, 0}, {0, 1}}
{ }

std::vector<MaxDistComputerOpt2::uint32> MaxDistComputerOpt2::computeAttribute(
  const MTree &tree) const 
{
  ...
  for (int level = 255; level >= 0; level--)  {
    ...
    for (NodePtr node: nodes) {
      ...
      if (!toRemove.empty())
        treeRemoval2(toRemove, stack, bin, Q_edt, root, pred, cost, N8);
    ...
    EDT_DIFF2(Q_edt, A8, N8, bin, root, pred, cost, Bedt);
...
std::array<std::vector<MaxDistComputerOpt2::NodePtr>, 256>
  MaxDistComputerOpt2::extractLevelMap(const MTree &tree) const 
{
...
void MaxDistComputerOpt2::initPredAndRoot(gft::sImage32 *pred, 
  gft::sImage32 *root) const 
{
...
void MaxDistComputerOpt2::insertNeighborsPQueue(
  uint32 pidx,
  gft::sImage32 *bin,
  gft::sImage32 *cost,
