...
// Useful function
std::vector<morphotree::uint32> computeMaxDistanceAttributeOpt(
  const morphotree::Box &domain,
  const std::vector<morphotree::uint8> &f,
  const MaxDistComputerOpt::MTree &tree)
{
  MaxDistComputerOpt comp{domain, f};
  return comp.computeAttribute(tree);
}

MaxDistComputerOpt::MaxDistComputerOpt(const Box& domain,
  const std::vector<uint8> &f)
  : domain_{domain}, f_{f}, 
    OFFSET_NEIGHBOR{ {-1, 0}, {0, -1}, {1, 0}, {0, 1}}
{ }

std::vector<MaxDistComputerOpt::uint32> MaxDistComputerOpt::computeAttribute(
  const MTree &tree) const 
{
  ...
  std::vector<int> f_eroded = erodeSerial(domain_, f_, Adj4{domain_});
...
std::array<std::vector<MaxDistComputerOpt::NodePtr>, 256>
  MaxDistComputerOpt::extractLevelMap(const MTree &tree) const 
{
...
gft::sImage32 *MaxDistComputerOpt::createGFTImage() const 
{
...
void MaxDistComputerOpt::initPredAndRoot(gft::sImage32 *pred, gft::sImage32 *root) const 
{
...
void MaxDistComputerOpt::insertNeighborsPQueue(
  uint32 pidx,
  gft::sImage32 *bin,
  gft::sImage32 *cost,
