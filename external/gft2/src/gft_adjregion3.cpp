...
namespace gft{
  namespace AdjRegion3{
    ...
    sAdjRegion3 *Create(int n){
      ...
      C = (sAdjRegion3 *) calloc(1,sizeof(sAdjRegion3));
      if(C != NULL){
	if(n>0){
	  C->d = (sDisplacement3 *)_mm_malloc(sizeof(sDisplacement3)*n, 16);
	  if(C->d==NULL)
	    gft::Error((char *)MSG1,
		       (char *)"AdjRegion3::Create");
	  memset((void *)C->d, 0, sizeof(sDisplacement3)*n);
	  C->n = n;
	}
	else{
	  C->d = NULL; 
    ...
    sAdjRegion3 *Create(sScene8 *mask,
			Voxel Ref){
      sAdjRegion3 *C;
      Voxel v;
      int i,p,n,nelems=0;
