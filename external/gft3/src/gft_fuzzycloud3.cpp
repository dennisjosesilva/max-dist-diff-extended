  ...
  namespace RegionCloud3{
	...
	*rcloud = NULL;
    ...
    sRegionCloud3 *Subsampling(sRegionCloud3 *rcloud){
      sRegionCloud3 *sub=NULL;
      sScene32 *tmp=NULL,*blur=NULL;
      int nobjs,l;
    ...
    sRegionCloud3 *GaussianBlur(sRegionCloud3 *rcloud){
      ...
      //Create RegionCloud3:
      blur = (sRegionCloud3 *)calloc(1,sizeof(sRegionCloud3));
      if(blur==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::GaussianBlur");
      blur->prob = (sScene32 **)calloc(nobjs+1,sizeof(sScene32 *));
      if(blur->prob==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::GaussianBlur");
    ...
    sRegionCloud3 *ChangeOrientationToLPS(sRegionCloud3 *rcloud,
					  char *ori){
      sRegionCloud3 *lps=NULL;
      int nobjs,l;
      
      nobjs = rcloud->nobjs;
      
      //Create RegionCloud3:
      lps = (sRegionCloud3 *)calloc(1,sizeof(sRegionCloud3));
      if(lps==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::ChangeOrientationToLPS");
      lps->prob = (sScene32 **)calloc(nobjs+1,sizeof(sScene32 *));
      if(lps->prob==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::ChangeOrientationToLPS");
	...
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::Read");
      rcloud->prob = (sScene32 **)calloc(nobjs+1,sizeof(sScene32 *));
      if(rcloud->prob==NULL)
	gft::Error((char *)MSG1,
		   (char *)"RegionCloud3::Read");
...
namespace gft{
  namespace BorderCloud3{
    ...
    sBorderCloud3 *ByRegionCloud(sRegionCloud3 *rcloud){
      ...
      //Create BorderCloud3:
      bcloud = (sBorderCloud3 *)calloc(1,sizeof(sBorderCloud3));
      if(bcloud==NULL)
	gft::Error((char *)MSG1,
	...
	*bcloud = NULL;
    ...
    sBorderCloud3 *Subsampling(sBorderCloud3 *bcloud){
      sBorderCloud3 *sub=NULL;
      sScene32 *tmp=NULL;
      int l;
      
      //Create BorderCloud3:
      sub = (sBorderCloud3 *)calloc(1,sizeof(sBorderCloud3));
      if(sub==NULL)
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::Subsampling");
      sub->prob = (sGradient3 **)calloc(bcloud->nobjs+1,sizeof(sGradient3 *));
      if(sub->prob==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::Subsampling");
    ...
    sBorderCloud3 *LinearInterp(sBorderCloud3 *bcloud,
      ...
      //Create BorderCloud3:
      interp = (sBorderCloud3 *)calloc(1,sizeof(sBorderCloud3));
      if(interp==NULL)
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::LinearInterp");
      interp->prob = (sGradient3 **)calloc(nobjs+1,
					   sizeof(sGradient3 *));
      if(interp->prob==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::LinearInterp");
    ...
    sBorderCloud3 *ChangeOrientationToLPS(sBorderCloud3 *bcloud,
					  char *ori){
      sBorderCloud3 *lps=NULL;
      int nobjs,l;
      nobjs = bcloud->nobjs;
      //Create BorderCloud3:
      lps = (sBorderCloud3 *)calloc(1,sizeof(sBorderCloud3));
      if(lps==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::ChangeOrientationToLPS");
      lps->prob = (sGradient3 **)calloc(nobjs+1,sizeof(sGradient3 *));
      if(lps->prob==NULL) 
	gft::Error((char *)MSG1,
		   (char *)"BorderCloud3::ChangeOrientationToLPS");
      lps->disp  = AdjRel3::Create(nobjs+1);
